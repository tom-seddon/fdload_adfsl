...
The decompression buffer is 4 KB (4096 bytes) (16 pages). These 16
pages have to be contiguous. In theory the buffer can overlap the
uncompressed data, as per zx02 docs.
...
otherwise. So while effects are running, all of $00...$ef is free, and
...
In zero page, only $FC is actually reserved. The MOS interrupt
preamble always overwrites it.

For page 2: the MOS interrupt preamble jumps through IRQ1V, so that's
fixed.

Keeping BRKV free is worthwhile as it'd be nice to have a useful BRK
routine.

It'd be good to have the =*FX200,3= flag at $0258 retained, so that
memory is wiped if pressing BREAK during the demo. Try to avoid
leaving too much of a mess behind.

$0290 and upwards is effectively free. 

There'll probably be an fdload_init entry point in page 2 somewhere,
that selects the fdload bank, copies the code to main RAM, and
restores old ROMSEL. Not too many bytes.
